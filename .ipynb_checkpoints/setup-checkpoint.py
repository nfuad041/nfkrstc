...
def main():
    ...
    Run this code in the LEGEND-base container:
    ...
    arg('--lh5_user', action=st, help='use $KRSTC_LH5_USER over $KRSTC_LH5')

    args = par.parse_args()

    # declare main DataGroup
    dg = DataGroup('$KRSTC_SW/data/krstc.json')
...
def init(dg):
    """
    ./setup.py --init
    Run first scan of the fileDB over the DAQ directory ($KRSTC_DAQ)
    """
    ...
    # compute lh5 column names (uses $KRSTC_LH5, don't use $KRSTC_LH5_USER here)
    dg.get_lh5_cols()
    ...
    # DROP MJ60 runs 
    print('WARNING: excluding early MJ60 data (cycles < 2019)')
    dg.fileDB = dg.fileDB.query('cycle >= 2019')
    dg.fileDB.reset_index(drop=True, inplace=True)
...
def update(dg, batch_mode=False):
    ...
    dg_new = DataGroup('$KRSTC_SW/processing/krstc.json')
    dg_new.scan_daq_dir()
...
def scan_orca_headers(dg, overwrite=False, batch_mode=False):
    ...
    if 'startTime' not in dg.fileDB.columns or overwrite:
        ...
        if len(idx) > 0:
            ...
            print('Found {len(df_keys)} new files without startTime:')
            print(df_keys)
...
def get_runtimes(dg, overwrite=False, batch_mode=False):
    ...
    NOTE: KRSTC uses struck channel 2 (0-indexed)
    """
