...
def pole_zero_fit_scipy(wf_in, A, tau, C):
    max_index = np.argmax(wf_in)+200
    last_index = len(wf_in)-100
    wf_in = wf_in[max_index:last_index]
    time = np.arange(max_index,last_index)
    try:
        pars, covs = curve_fit(exp_decay_func, time,wf_in, p0=[50,50,0])
        A[0] = pars[0]
        tau[0] = pars[1]
        C[0] = pars[2]
