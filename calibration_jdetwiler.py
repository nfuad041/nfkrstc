#Original file by J. Detwiller.Copied from github on 2021-05-25. https://github.com/jasondet/pygama/blob/dev/pygama/analysis/calibration.py
...
def hpge_fit_E_peak_tops(hist, bins, var, peak_locs, poissonLL=[], n_to_fit=7,
                          inflate_errors=False, gof_method='var'):
    ...
    for E_peak in peak_locs:
        ...
        #poissonLL=poissonLL[i]
        #print(poissonLL[i])
        if poissonLL is None:
            poissonLL = False
        pars, cov = pgp.gauss_mode_width_max(hist, bins, var, 
                                             mode_guess=E_peak, 
                                             n_bins=n_to_fit, 
...
def match_peaks(data_pks, cal_pks):
    ...
    print("i best_err : ",i, best_err)
    print("cal_shape, cal:",cal.shape, cal)
    
    print("data_shape, data:",data.shape,data)
...
def calibrate_tl208(energy_series, cal_peaks=None, plotFigure=None):
    ...
    if cal_peaks is None:
        ...
        cal_peaks = np.array([238.6,338.3,463.0, 511.0, 583.2, 727.3, 794.9, 860.6, 911.2, 969.0, 1460.8, 1592.5, 2103.5, 2614.5])
    ...
    #peak_energies, peak_e_err = get_most_prominent_peaks(energy_hi, min_adc, max_adc, 10)
    peak_energies = get_most_prominent_peaks(energy_hi, min_adc, max_adc, 10)
    ...
    for i, energy in enumerate(cal_peaks):
        ...
        try:
            guess_e, guess_sigma, guess_area = pgh.get_gaussian_guess(peak_hist, bin_centers)
        except IndexError:
